	...
	$.initialize(".list-view-header", function() {
		...
		$('#check-prices').on('click', function() {
			...
			$('.itemList .listFUTItem').each(function() {
				...
				player_html = searchPlayer(player_name, player_rating, player_position, price_area);
			});
...
function searchPlayer(player_name, player_rating, player_position, price_area) {
	$.ajax({
		type: 'GET',
		url: 'https://www.futbin.com/search?year=18&term=' + player_name + '&_=1514602993894',
	}).done(function(data) {
		$.each(data, function(key, value) {
			futbin_full_name = value.full_name;
			futbin_name = value.name;
			futbin_id = value.id;
			...
			if(futbin_rating == player_rating && futbin_position == player_position && futbin_name == player_name) {
				getPlayerPricing(resource_id, futbin_full_name, futbin_id, price_area);
			}
		});
	});
}
function getPlayerPricing(resource_id, futbin_full_name, futbin_id, price_area) {
	$.ajax({
		type: 'GET',
		...
		player_html = '<div id="DIV_1">\
							...
							<button class="standard section-header-btn mini call-to-action futbin_link" link="https://www.futbin.com/18/player/' + futbin_id + '/' + futbin_full_name + '/">\
								<img src="https://cdn.futbin.com/design/img/small-futbin-logo.png?v=5">\
							</button>';
		console.log(player_html);
		price_area.append(player_html);
	})
;}